...
export function AideSearchHome() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setLoading(true);
      fetch(`/api/aide/search?q=${encodeURIComponent(query)}`)
        .then((r) => r.json())
  ...
  return (
    <div ref={containerRef} className="relative max-w-xl mx-auto">
      ...
      {showDropdown && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-3xl border border-gray-200 shadow-lg overflow-hidden z-50">
          {loading ? (
            <div className="px-4 py-6 text-center text-anthracite/60 text-sm">Recherche...</div>
          ) : results.length > 0 ? (
            <ul className="py-2">
              {results.map(({ profile, category, article }) => (
                <li key={`${profile}-${category.id}-${article.slug}`}>
                  <Link
